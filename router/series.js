...
router.get('/getTop', authenticateJWT , async (req, res) => {
  ...
  try {
    ...
    // execute query with page and limit values
    const data = await Tvseriestitledata.find( query1 ,{ __v:0 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort(sortBy)
      .exec();
    ...
    // get total documents in the Posts collection 
    const count = await Tvseriestitledata.countDocuments(query1);
...
// Pagination movie table
router.get('/gettitle', authenticateJWT , async (req, res) => {
  // destructure page and limit and set default values
  const { title= ""} = req.query;
  try {
    const query1 = { "title" : title }
    const data = await Tvseriesdata.findOne( query1 ,{__v:0 , created_at:0, topick:0 })
    res.json({
      data : data,
    });
  } catch (err) {
...
router.get('/getid', authenticateJWT , async (req, res) => {
  ...
  try {
    const query1 = { "_id" : id }
    // execute query with page and limit values
    const data = await Tvseriestitledata.findOne( query1 ,{__v:0 , created_at:0, topick:0 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort(sortBy)
      .exec();
...
router.get('/getgenre', authenticateJWT , async (req, res) => {
  ...
  try {
    ...
    // execute query with page and limit values
    const data = await Tvseriestitledata.find( query1 ,{__v:0 , created_at:0, topick:0 })
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort(sortBy)
      .exec();
...
router.get('/genre' , async (req, res) => {
  try {
    var genre = []
    const data = await Tvseriestitledata.aggregate([
        { $match: { }},
        { $unwind: '$genre' },
        { $group: { _id: '$genre'}}
      ])
...
router.get('/search', authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    // execute query with page and limit values
    const data = await Tvseriestitledata.find(query1,{__v:0 , created_at:0})
      .limit(limit * 1)
      .skip((page - 1) * limit)
      .sort(sortBy)
      .exec();
